...
const categories = (...counts: number[]): Category[] =>
  counts.map((count, i) => ({ name: `cat-${i}`, services: services(count, `cat-${i}-svc`) }))
...
describe('useDashboardLayout defaults', () => {
  ...
  it('derives visible rows from the service count, capped so tall categories scroll', () => {
    ...
    expect(result.current.layout.categories.lg?.map(p => p.rows)).toEqual([1, 2, 3])
  })

  it('lays tiles out row-major within the category width', () => {
    const { result } = render(categories(6))
    expect(result.current.layout.tiles['cat-0'].lg?.map(t => [t.x, t.y])).toEqual([
      [0, 0],
      [1, 0],
      [2, 0],
      [3, 0],
    ...
    expect(result.current.layout.tiles['cat-0'].xs?.map(t => [t.x, t.y])).toEqual([
      [0, 0],
      [1, 0],
      [2, 0],
      [0, 1],
...
describe('useDashboardLayout precedence', () => {
  it('honours w and rows from the category config', () => {
    ...
    expect(result.current.layout.categories.lg?.[0]).toMatchObject({ w: 6, rows: 1 })
    // Six columns of tiles, so the sixth tile stays on the first row.
    expect(result.current.layout.tiles['cat-0'].lg?.[5]).toMatchObject({ x: 5, y: 0 })
  })
  ...
  it('lets a per-breakpoint layout override the category-wide keys', () => {
    ...
    expect(result.current.layout.categories.md?.[0]).toMatchObject({ w: 2, rows: 3 })
  })
  ...
  it('prefers a saved layout over the config block', () => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        layout: { categories: { lg: [{ i: 'cat-0', x: 4, y: 9, w: 1, rows: 4 }] } }
      })
    )
  ...
  it('discards a saved layout from an earlier version or a changed config', () => {
    ...
    expect(result.current.isCustomized).toBe(false)
    expect(result.current.layout.categories.lg?.[0]).toMatchObject({ x: 0, w: 4 })
  })
  ...
  it('drops saved entries for categories and services that no longer exist', () => {
    ...
    expect(result.current.layout.categories.lg?.map(p => p.i)).toEqual(['cat-0'])
...
describe('useDashboardLayout persistence', () => {
  it('persists a category change and marks the layout customised', () => {
    ...
    expect(saved.layout.categories.lg[0]).toEqual({ i: 'cat-0', x: 2, y: 0, w: 3, rows: 2 })
  })
  ...
  it('persists a tile change under its category and breakpoint only', () => {
    ...
    expect(result.current.layout.tiles['cat-0'].lg?.[1]).toMatchObject({ w: 2, h: 2 })
    // Other breakpoints keep their generated layout.
    expect(result.current.layout.tiles['cat-0'].md?.[1]).toMatchObject({ w: 1, h: 1 })
  })

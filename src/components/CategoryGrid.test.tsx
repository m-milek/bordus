...
const cardIn = (item: HTMLElement) => item.querySelector('.category-card') as HTMLElement
...
describe('CategoryGrid', () => {
  it('gives each category a height of exactly its whole tile rows plus the gap strip', () => {
    ...
    expect(parseFloat(media.style.height) - parseFloat(cardIn(media).style.height)).toBe(
      CAT_GAP
    )
    expect(
      parseFloat(downloads.style.height) - parseFloat(cardIn(downloads).style.height)
    ).toBe(CAT_GAP)
  })
  ...
  it('places categories on the same column grid the tiles use', () => {
    ...
    const left = (name: string) =>
      parseFloat(itemFor(name).style.transform.match(/translate\((-?[\d.]+)px/)![1])
  ...
  it('keeps one unbroken tile rhythm across a category boundary', () => {
    ...
    const span = (tile: string) => {
      const item = screen.getByText(tile).closest('.react-grid-item') as HTMLElement
      const left = offset(item.closest('.category-item') as HTMLElement) + offset(item)
  ...
  it('hides resize handles until edit mode is on', () => {
    ...
    const { container, unmount } = setup([{ i: 'Media', x: 0, y: 0, w: 4, rows: 1 }])
    ...
    const edit = setup([{ i: 'Media', x: 0, y: 0, w: 4, rows: 1 }], { isEditMode: true })

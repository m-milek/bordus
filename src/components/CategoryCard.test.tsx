...
const tileFor = (name: string) => screen.getByText(name).closest('.react-grid-item') as HTMLElement
...
const box = (element: HTMLElement) => {
  const [, left] = element.style.transform.match(/translate\((-?[\d.]+)px,(-?[\d.]+)px\)/) ?? []
  const [, , top] = element.style.transform.match(/translate\((-?[\d.]+)px,(-?[\d.]+)px\)/) ?? []
  return {
...
describe('CategoryCard geometry', () => {
  it('places tiles at the offsets the page-wide grid pitch dictates', () => {
    render(
      <CategoryCard category={category(6)} width={4} rows={2} cell={CELL} tiles={grid(6, 4)} />
    )
  ...
  it('leaves exactly one gutter between tiles and none at the block edges', () => {
    render(
      <CategoryCard category={category(4)} width={4} rows={1} cell={CELL} tiles={grid(4, 4)} />
    )
  ...
  it('keeps the same cell size and column boundaries whatever the category width', () => {
    const { unmount } = render(
      <CategoryCard category={category(2)} width={2} rows={1} cell={CELL} tiles={grid(2, 2)} />
    )
    const narrow = [0, 1].map(i => box(tileFor(`svc-${i}`)))
    unmount()

    render(
      <CategoryCard category={category(2)} width={6} rows={1} cell={CELL} tiles={grid(2, 6)} />
    )
  ...
  it('sizes the tile block to the grid item box, and the card to overhang it', () => {
    const { container } = render(
      <CategoryCard category={category(4)} width={4} rows={2} cell={CELL} tiles={grid(4, 4)} />
    )
  ...
  it('offers one snap stop per hidden row, the last at the maximum scroll offset', () => {
    ...
    const { container } = render(
      <CategoryCard category={category(9)} width={4} rows={2} cell={CELL} tiles={grid(9, 4)} />
    )

    const stops = Array.from(
      container.querySelectorAll<HTMLElement>('.category-body > [aria-hidden="true"]')
  ...
  it('does not scroll at all when every tile already fits', () => {
    ...
    expect(container.querySelectorAll('.category-body > [aria-hidden="true"]')).toHaveLength(0)
  ...
  it('reorders a full row in place instead of spilling onto a new one', () => {
    ...
    const three: Category = {
      name: 'Media',
      services: ['one', 'two', 'three'].map(n => ({ name: n, url: `http://${n}.local` }))
    }
    const { container } = render(
      <CategoryCard
        category={three}
  ...
  it('renders the title pill with the category icon', () => {
    render(
      <CategoryCard category={category(2)} width={4} rows={1} cell={CELL} tiles={grid(2, 4)} />
    )
